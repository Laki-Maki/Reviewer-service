...
Проблема: Я не знал может ли пользователь быть в нескольких командах или он может быть закреплён только за одной.
Решение: Пользователь может состоять в нескольких командах.
В сервисе один и тот же пользователь может быть членом нескольких команд одновременно. Это позволяет гибко назначать ревьюверов из разных команд и учитывать их активность при распределении нагрузки. При этом логика назначения PR всегда учитывает команду, в которой был создан PR, чтобы распределение оставалось корректным и предсказуемым.
...
Добавил метод для управление активностью пользователя. Статус пользователя (is_active) можно менять с true на false.

Во всех таблицах и моделях для идентификаторов (id, author_id, reviewer_id) используется целочисленный тип int, а не строка, как указано в документации. Я заметил это слишком поздно и сожалею, что не учёл это раньше... (Тем не менее, использование int упрощает работу с БД и ускоряет выборки, сортировки и подсчёт нагрузки на участников.)
